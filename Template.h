...
/*Bash博弈:有一堆物品共n个，两个人轮流从这堆物品中取物，规定每次至少取一个最多取m个,最后取光者得胜。
**返回参数若为true则先手胜，若为false的后手胜
***解释:若n=m+1则不论先手拿走多少个，后手皆可一次拿完2
***    若n=(m+1)x+s则先手拿走s个后，后手拿走k个(k<=m)，先手拿走m+1-k个，最终先手必胜
***    可以看出若n%(m+1)!=0则先手必胜，反之先手必败
*/
bool Template::Bash_game(int n, int m)
{
	return n % (m + 1) != 0;
}
